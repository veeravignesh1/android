...
class RepositoryTest : StringSpec() {
    ...
    init {
        ...
        "startTimeEntry stops currently running time entry and inserts a new one to DAO" {
            ...
            val startedTimeEntries = DatabaseTimeEntry(
                5,
                "started",
                timeEntryOneStartTime,
                null,
            ...
            every { timeService.now() } returns nowTime
            every { timeEntryDao.startTimeEntry(any(), any(), any()) } returns (startedTimeEntries to stoppedTimeEntries)

            val result = repository.startTimeEntry(startedTimeEntries.id, startedTimeEntries.description)

            verify(exactly = 1) {
                timeEntryDao.startTimeEntry(startedTimeEntries.id, startedTimeEntries.description, nowTime)
            }
            verify {
                workspaceDao wasNot called
                clientDao wasNot called
            ...
            result shouldBe StartTimeEntryResult(
                startedTimeEntries.let(DatabaseTimeEntry::toModel),
